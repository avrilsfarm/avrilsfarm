  ...
  el.innerHTML = `
    <div class="page-header">
      ...
      <div style="display:flex;gap:6px">
        <button class="header-btn" onclick="openTRFromBatch()" style="background:var(--teal);color:#fff;border-color:var(--teal);padding:6px 10px;font-size:12px">
          시험성적서
        </button>
        <button class="header-btn" onclick="openProductMasterList()" style="background:var(--mauve);color:#fff;border-color:var(--mauve);padding:6px 10px;font-size:12px">
          제품표준서
        </button>
      </div>
    </div>
...
async function openProductMasterForm(id) {
  ...
  const nextPsNo = () => {
    const nums = products.map(p => p.문서번호 && p.문서번호.match(/^AF-PS-(\d+)$/))
  ...
  showSheet(`
    ...
    <label>제조번호 형식<input id="pm14" value="${item?.제조번호형식||((typeof mfgPrefix==='function'?mfgPrefix():'APB')+'O')}" placeholder="예: ${(typeof mfgPrefix==='function'?mfgPrefix():'APB')}O"></label>
